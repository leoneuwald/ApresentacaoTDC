...
import br.com.tdc.model.AproveitamentoUltimosJogos;
import br.com.tdc.model.Brasileirao;
import br.com.tdc.model.Classificacao;
import br.com.tdc.model.Jogo;
...
public class DadosDoCampeonato {
	...
	private static void timesNoRebaixamento(Brasileirao brasileirao) {
		...
		brasileirao.getClassificacao().stream()
			.sorted(Comparator.comparing(Classificacao::getPontos)
...
//		
//		brasileirao.getClassificacao().stream()
//			.map(j -> j.getAproveitamento_ultimos_jogos())
//			.forEach((j) -> {j.stream()
//				.filter(x -> "v".equals(x.getAproveitamento().trim()))
//				.filter(x -> "visitante".equals(x.getJogo().getVencedor_jogo().getLabel()))
//				.forEach(y -> System.out.println(
//						"Vencedor: " + y.getJogo().getEquipe_visitante().getNome_popular()));
//			});
//
//		System.out.println(" ");
		...
		brasileirao.getClassificacao().stream()
			...
			.sorted(
					(t1, t2) ->  t1.getEquipe_visitante().getNome_popular().compareTo( t2.getEquipe_visitante().getNome_popular() )
			)
//			.forEachOrdered(eq -> System.out.println(eq.getEquipe_visitante().getNome_popular()));
//			.forEachOrdered(System.out::println);
			
			
			.collect(Collectors.groupingBy(	
					x -> x.getEquipe_visitante().getNome_popular(), Collectors.counting() )
					);
			
//			.collect(Collectors.mapping(x -> x.getEquipe_visitante().getNome_popular(), Collectors.counting()));
			...
			.forEach(
					(j) -> {
						...
						List<AproveitamentoUltimosJogos> aproveitamentoUltimosJogos = j.stream()
								.filter(x -> "v".equals(x.getAproveitamento().trim()))
								.filter(x -> "COR".equals("mandante".equals(x.getJogo().getVencedor_jogo().getLabel())
										? x.getJogo().getEquipe_mandante().getSigla()
										: x.getJogo().getEquipe_visitante().getSigla()))
								// .sorted(Comparator.comparing(x ->
								// x.getJogo().getData_realizacao()))
								.collect(Collectors.toList());
...
//								j.stream()
//									.filter(x -> "v".equals(x.getAproveitamento().trim()))
//									.filter(x ->  "COR".equals( "mandante".equals(x.getJogo().getVencedor_jogo().getLabel()) ? x.getJogo().getEquipe_mandante().getSigla() : x.getJogo().getEquipe_visitante().getSigla() ) )
//									.sorted(Comparator.comparing(x -> x.getJogo().getData_realizacao()))
//									.collect(Collectors.toList()) 
//								);
							}
