...
import br.com.tdc.model.Artilharia;
...
public class DadosDeArtilharia {
	...
	private static void artilheirosDoGremioOrdenadosPorNumeroDeGols(List<Artilharia> artilharia) {
		...
		for (Artilharia art : artilheirosDoGremio) {
			System.out.format("%-2s%-4s%-20s%-4s%-20s%n", art.getTotal_gols(), " - ", art.getJogador_nome(), " - ", art.getJogador_posicao());			
		}
		...
		artilharia.stream()
			...
			.forEach(a -> System.out.format("%-2s%-4s%-20s%-4s%-20s%n", a.getTotal_gols(), " - ", a.getJogador_nome(), " - ", a.getJogador_posicao()));
	...
	private static void artilhariaDoCampeonato(List<Artilharia> artilharia) {
		...
		for (Artilharia art : artilheirosDoCampeonato) {
			System.out.format("%-2s%-4s%-20s%-4s%-20s%-4s%-20s%n", art.getTotal_gols(), " - ", art.getJogador_nome(), " - ", art.getJogador_posicao(), " - ", art.getEquipe_nome());
		}
		...
		artilharia.stream()
			.sorted(Comparator.comparing(Artilharia::getTotal_gols).reversed())
			.forEach(
				a -> System.out.format("%-2s%-4s%-20s%-4s%-20s%-4s%-20s%n", a.getTotal_gols(), " - ", a.getJogador_nome(), " - ", a.getJogador_posicao(), " - ", a.getEquipe_nome())
				);
	...
	private static void artilheirosAtacantes(List<Artilharia> artilharia) {
		...
		atacantes.forEach(a -> 
			System.out.format("%-2s%-4s%-20s%-4s%-14s%n", a.getTotal_gols(), " - ", a.getJogador_nome(), " - ", a.getEquipe_nome())
		);
		...
		Map<String, Long> atacantesPorTime = artilharia.stream()
			...
			.collect(
					Collectors.groupingBy(
							Artilharia::getEquipe_nome, Collectors.counting()) 
				);
		...
		long totalDeGols = artilharia.stream()
			.filter(j -> "Atacante".equals(j.getJogador_posicao()))
			.mapToLong(Artilharia::getTotal_gols).sum();	
		...
		Optional<String> findFirst = Stream.of("Whatever", "it", "takes", "to", "break", "Gotta", "do", "it", 
